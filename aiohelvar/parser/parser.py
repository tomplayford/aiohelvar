...
class CommandParser():
    ...
    def parse_command(self, input):
        ...
        if match is None:
            raise UnrecognizedCommand(input, "Could not locate a valid command in input.")
        ...
        return Command(command_type, parameters, MessageType(match.group("type")), address)

    def parse_command_type(self, match):
        try:
            return CommandType.get_by_command_id(int(match.group("command")))
        except KeyError:
            raise UnrecognizedCommand(self.raw_command, f"Did not recognize Command Type: {match.group('command')}")
    ...
    def parse_params(self, match):
        ...
        if match.group('params'):
            ...
            for param in params:
                ...
                if len(parts) == 2:
                    try:
                        parameters.append(CommandParameter(CommandParameterType(parts[0]), parts[1]))
                    except ValueError:
                        # logger.debug(f"Unsupported Parameter: {param}")
                        raise UnrecognizedCommand(self.raw_command, f"Unsupported Parameter: {param}")
                else:
                    # logger.debug("Couldn't identify parameter pair in string: {}", param)
                    raise UnrecognizedCommand(self.raw_command, f"Couldn't identify parameter pair in string: {param}")
